...
function tlsFingerprint(req) {
  ...
  // (Chrome UA + python JA3) is a strong tell, and for years it was close to a
  // free win. It no longer is. `curl_cffi`, `primp`, `hrequests` and
  // `curl-impersonate` reproduce a real Chrome/Firefox/Safari ClientHello —
  // cipher order, extensions, curves — so a scraper that has bothered simply
  // does not mismatch. Treat JA3/JA4 as one weighted input (guard 47), not as
  // a verdict: it catches the unprepared and nobody else.
...
function h2Fingerprint(req) {
  ...
  return {
    ...
    // The heuristic below — "claims Chrome, speaks HTTP/1.1" — is deliberately
    // weak, and named as such so nobody mistakes it for the real control.
    // `httpx` and `curl_cffi` negotiate HTTP/2 natively, so they sail past it.
    // The actual guard is the *shape* of the connection: SETTINGS values and
    // their order, the initial WINDOW_UPDATE, the PRIORITY tree and the
    // pseudo-header order — which is what the `akamaiFp` string above encodes
    // and what an edge proxy would compare against a known-browser table.
